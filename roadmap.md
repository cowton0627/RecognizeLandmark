...
> 2026-07-22 完成競品與產品方向研究，詳見 [PRODUCT_RESEARCH.md](./PRODUCT_RESEARCH.md)。產品定位收斂為「隱私優先、低摩擦、自動辨識的地標收藏簿」。執行順序以研究文件的 P0 → P1 → P2 為準；以下既有項目保留作為功能細節池。

> 若目標是近期面試展示，改以 [INTERVIEW_SHOWCASE.md](./INTERVIEW_SHOWCASE.md) 的 Showcase MVP 為優先：實機穩定 → Photos 匯入 → 足跡地圖 → 測試／migration → README 展示素材。Stage 6b 動畫與統計排在其後。

### P0:核心可靠性與可修正性 → **下一個**

- [ ] 實機走完相機／位置／離線／取消／刪除等主要與失敗流程（清單見 `runbook.md`）
- [x] 確認頁加入重新搜尋與手動選地圖位置，避免辨識錯誤無法修正
- [x] 把候選 ranking 從固定來源順序升級成距離、來源、影像信心的可測策略
- [x] 以開發版本機資料記錄有候選率、首選採用率、手動更名率與保存耗時
- 完成後再進 Stage 6b；其中「最近記錄 strip」優先於純裝飾動畫

### Stage 6b:相機首頁手繪風動態元素
